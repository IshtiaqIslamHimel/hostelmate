...
export default function AdminTasks(){
  ...
  const addTarget = (id:string) => { if(!f.targets.includes(id)) setF({...f, targets:[...f.targets, id]}) }
  const removeTarget = (id:string) => setF({...f, targets: f.targets.filter(x=>x!==id)})
  const moveTarget = (idx:number, dir:-1|1) => {
    const arr = [...f.targets]; const j = idx + dir
  ...
  const save = async ()=>{
    ...
    try {
      const r = await adminFetch('/api/admin/create-task', {
        ...f,
        intervalDays: Number(f.intervalDays),
        fineAmount: Number(f.fineAmount)||0
      })
      setSaveMsg(`Task created! Telegram notified ${r.notified || 0} people.` + (r.group_posted ? ' + Group posted.' : ''))
  ...
  return <AppShell>
    ...
    <div className="card mb-4">
      ...
      <div className="grid md:grid-cols-2 gap-2">
        ...
        <div><label className="label">Late Fine ৳</label><input type="number" className="input" min={0} value={f.fineAmount} onChange={e=>setF({...f, fineAmount: parseInt(e.target.value)||0})} /></div>
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-4 mt-4">
        <div>
          ...
          <div className="border border-slate-200 rounded-xl p-2 max-h-56 overflow-auto bg-slate-50">
            ...
            {targetList.filter((x:any)=>!f.targets.includes(x.id)).length===0 && <div className="text-slate-500 text-sm px-2">All added</div>}
          </div>
        </div>
        <div>
          <div className="label">Rotation Order – {f.targets.length} selected</div>
          <div className="border border-slate-200 rounded-xl p-2 max-h-56 overflow-auto bg-white">
            {f.targets.length===0 && <div className="text-slate-500 text-sm px-2">Click names on the left to add in order.</div>}
            {f.targets.map((id, idx)=>(
              <div key={id} className="flex items-center justify-between px-2 py-1.5 border-b border-slate-100 last:border-0 text-sm">
                ...
                <div className="flex gap-1">
                  <button type="button" className="px-2 py-0.5 text-xs bg-slate-100 rounded disabled:opacity-30" disabled={idx===0} onClick={()=>moveTarget(idx,-1)}>↑</button>
                  <button type="button" className="px-2 py-0.5 text-xs bg-slate-100 rounded disabled:opacity-30" disabled={idx===f.targets.length-1} onClick={()=>moveTarget(idx,1)}>↓</button>
      ...
      {preview.length > 0 && (
        <div className="mt-3 p-3 bg-slate-50 rounded-xl border border-slate-200">
          <div className="text-xs font-bold text-slate-600 mb-1">Preview – next {preview.length} occurrences {f.fineAmount>0 && `· Fine ৳${f.fineAmount}`}</div>
          <div className="text-xs text-slate-600 max-h-36 overflow-auto">{preview.slice(0,12).map(p=><div key={p.date}>{p.date} → {p.assignees.map(a=> nameOf(a, f.assignType)).join(', ')}</div>)} {preview.length>12 && <div>… {preview.length-12} more</div>}</div>
        </div>
      )}

      <div className="flex items-center gap-3 mt-3">
        <button className="btn" disabled={saving} onClick={save}>{saving ? 'Creating…' : 'Save Task & Notify'}</button>
        {saveMsg && <span className="text-sm text-emerald-700">{saveMsg}</span>}
      </div>
      <p className="text-xs text-slate-500 mt-2">Saving sends Telegram DM to all assigned members + posts to the hostel group. Members also get notified 7 days before + morning of duty.</p>
    </div>

    <div className="card overflow-x-auto">
      <table className="w-full"><thead><tr><th>Task</th><th>Assign</th><th>Repeat</th><th>Fine</th><th></th></tr></thead>
        <tbody>
          {tasks.map((t:any)=> <tr key={t.id} className="hover:bg-slate-50">
            <td><button onClick={()=>openDetail(t.id)} className="text-left"><b className="text-brand hover:underline">{t.title}</b><div className="text-slate-500 text-xs">{t.description||'—'}</div></button></td>
            ...
            <td><button className="btn btn-secondary !py-1 !px-2 text-xs mr-2" onClick={()=>openDetail(t.id)}>Details</button>
            <button className="btn btn-red !py-1 !px-2 text-xs" onClick={async()=>{ if(confirm('Delete?')){ await deleteDoc(doc(db,'tasks',t.id)); load()}}}>Delete</button></td>
          </tr>)}
        </tbody>
      </table>
    </div>
...
function TaskDetailModal({task, users, rooms, onClose}:{task:any, users:any[], rooms:any[], onClose:()=>void}) {
  ...
  const nameOf = (id:string) => task.assignType==='member' ? (users.find(u=>u.id===id)?.name||id) : (rooms.find(r=>r.id===id)?.name||id)
  useEffect(()=>{ import('firebase/firestore').then(async ({collection, query, where, getDocs})=>{ const {db}=await import('@/lib/firebaseClient'); const snap=await getDocs(query(collection(db,'completions'), where('taskId','==',task.id))); setCompletions(snap.docs.map(d=>d.data()).sort((a:any,b:any)=>(b.doneAt||'').localeCompare(a.doneAt||''))) }) }, [task.id])
  const move = (i:number,d:-1|1)=>{ const a=[...targets]; const j=i+d; if(j<0||j>=a.length) return; [a[i],a[j]]=[a[j],a[i]]; setTargets(a) }
  const save = async()=>{ setSaving(true); const {doc,updateDoc}=await import('firebase/firestore'); const {db}=await import('@/lib/firebaseClient'); await updateDoc(doc(db,'tasks',task.id), {targets, fineAmount:Number(fineAmount)||0}); setSaving(false); alert('Saved'); onClose() }
  ...
  <div className="fixed inset-0 bg-black/40 z-50 grid place-items-center p-4" onClick={onClose}>
    <div className="card w-full max-w-2xl max-h-[90vh] overflow-auto" onClick={e=>e.stopPropagation()}>
      <div className="flex justify-between items-start mb-2"><div><h3 className="font-extrabold text-lg">{task.title}</h3><div className="text-slate-500 text-sm">{task.description||'—'}</div></div><button onClick={onClose} className="text-slate-500">✕</button></div>
      <div className="text-xs text-slate-600 mb-3">{task.assignType} · {task.rotation} · {task.repeat} · Start {task.startDate} · Fine ৳{task.fineAmount||0}</div>
      <div className="grid md:grid-cols-2 gap-4">
        <div>
          ...
          <div className="border border-slate-200 rounded-xl max-h-56 overflow-auto mb-2">
            {targets.map((id,idx)=><div key={id} className="flex items-center justify-between px-3 py-2 border-b border-slate-100 last:border-0 text-sm"><span><b className="text-slate-400 mr-2">{idx+1}.</b> {nameOf(id)}</span><div className="flex gap-1"><button className="px-2 py-0.5 text-xs bg-slate-100 rounded disabled:opacity-30" disabled={idx===0} onClick={()=>move(idx,-1)}>↑</button><button className="px-2 py-0.5 text-xs bg-slate-100 rounded disabled:opacity-30" disabled={idx===targets.length-1} onClick={()=>move(idx,1)}>↓</button><button className="px-2 py-0.5 text-xs text-red-600" onClick={()=>setTargets(t=>t.filter(x=>x!==id))}>×</button></div></div>)}
          </div>
        ...
        <div>
          <div className="font-bold text-sm mb-1">Upcoming</div>
          <div className="border rounded-xl p-2 max-h-48 overflow-auto text-xs bg-slate-50 mb-3">{preview.map(p=> <div key={p.date}>{p.date} → {p.assignees.map(a=>nameOf(a)).join(', ')}</div>)}</div>
          <div className="font-bold text-sm mb-1">Recent Completions</div>
          <div className="border rounded-xl p-2 max-h-32 overflow-auto text-xs bg-slate-50">{completions.slice(0,20).map((c:any,i)=><div key={i}>{c.date} · {nameOf(c.assigneeKey)} · {c.done?'✓':''}</div>)}{completions.length===0 && 'None'}</div>
        </div>
      </div>
    </div>
  </div>)
